package demos.compare
{
	 ...
	 */
	public class SpreadMethodMain extends Sprite
	{
		/**
		 * 构造函数
		 * create a [SpreadMethodMain] object
		 */
		public function SpreadMethodMain()
		{
			var list:Vector.<Shape> = new Vector.<Shape>();
			list.push(new GradientRegion(SpreadMethod.PAD));
			list.push(new GradientRegion(SpreadMethod.REFLECT));
			list.push(new GradientRegion(SpreadMethod.REPEAT));
...
class GradientRegion extends Shape
{
	public function GradientRegion(method:String)
	{
		...
		graphics.lineStyle();
		graphics.beginGradientFill(GradientType.LINEAR, [0xFF0000, 0xFFFF00, 0x00FF00], [1,1,1], [0, 0x7F, 0xFF], matrix, method);
